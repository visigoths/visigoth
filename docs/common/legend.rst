
======
Legend
======

.. automodule:: visigoth.common.legend

.. autoclass:: visigoth.common.legend.Legend

-------
Example
-------

.. raw:: html

    <iframe src="../_static/src/common/legend/example.html" height="512px" width="100%"></iframe>

`(Link to open example in new tab) <../_static/src/common/legend/example.html>`_:


Source Code `(Link to Github) <https://github.com/visualtopology/visigoth/blob/master/docs/src/common/legend/example.py>`_:

.. literalinclude:: ../_static/src/common/legend/example.py
    :language: python




